...
function useObrasStore() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get("carpinova-obras");
        const parsed = JSON.parse(r.value);
        setObras(Array.isArray(parsed) && parsed.length ? parsed : SEED_DATA);
        if (!Array.isArray(parsed) || !parsed.length) {
          await window.storage.set("carpinova-obras", JSON.stringify(SEED_DATA));
        }
      } catch (e) {
        setObras(SEED_DATA);
        try { await window.storage.set("carpinova-obras", JSON.stringify(SEED_DATA)); } catch (e2) {}
      } finally {
        setLoading(false);
      }
    })();
  ...
  const persist = useCallback(async (next) => {
    ...
    setSaveState("saving");
    try {
      await window.storage.set("carpinova-obras", JSON.stringify(next));
      setSaveState("saved");
  ...
  const addObra = useCallback((partial) => {
    setObras((cur) => {
      const novaObra = {
        id: uid(), ref: nextRef(cur), cliente: "", projeto: "", descricao: "", canal: "",
  ...
  const updateObra = useCallback((id, patch) => {
    setObras((cur) => {
      const next = cur.map((o) => (o.id === id ? { ...o, ...patch } : o));
      persist(next);
      return cur;
    });
  }, [persist]);
  ...
  const changeEstado = useCallback((id, novoEstado) => {
    setObras((cur) => {
      const alvo = cur.find((o) => o.id === id);
      if (!alvo || alvo.estado === novoEstado) return cur;
      const antiga = stageOf(alvo.estado).label;
      const nova = stageOf(novoEstado).label;
      const next = cur.map((o) => (o.id === id ? {
        ...o, estado: novoEstado,
        historico: [...(o.historico || []), { data: todayISO(), texto: `Estado alterado: ${antiga} → ${nova}.` }],
      } : o));
      persist(next);
      return cur;
    });
  ...
  const addHistorico = useCallback((id, texto) => {
    setObras((cur) => {
      const next = cur.map((o) =>
        o.id === id ? { ...o, historico: [...(o.historico || []), { data: todayISO(), texto }] } : o
      );
      persist(next);
      return cur;
    });
